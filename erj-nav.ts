...
interface ErjChild {
  ...
  keys?: string[]; external?: boolean; learnMore?: boolean;
}
interface ErjTop {
...
(() => {
  ...
  function hereBlock(label: string): string {
  ...
  function childRow(c: ErjChild): string {
    if (matches(c.keys)) return hereBlock(c.label); // current page: anchors, not a duplicate link
  ...
  function wire(): void {
    ...
    panel.querySelectorAll('.erj-glabel').forEach(btn => {
      btn.addEventListener('click', () => {
        const g = (btn as HTMLElement).parentElement;
        if (!g) return;
